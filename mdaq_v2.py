...
tx_array.append(0x000000FF & mon_period)                 
tx_array.append((0x0000FF00 & mon_period)>>8)
tx_array.append((0x00FF0000 & mon_period)>>16)  
tx_array.append((0xFF000000 & mon_period)>>24)   
ser.write(bytes(tx_array))
...
def send_LED_cube(bitstring, box_info, mapping, bit_low, bit_high):
...
def send_LED_cube_animate(bitstring, box_info, mapping, bit_low, bit_high):
    ser = serial.Serial('/dev/ttyS0', baudrate=9600)
...
while 1:
    ...
    if(len(data) == 8):
        ...
        tailer = ((rx_array[7] & 0x00ff) << 8) | (rx_array[6] & 0x00ff)
        if(header == 0xa5a5 and tailer == 0xd5d5):
            eve_word = ((rx_array[5] & 0x000000ff) << 24) | ((rx_array[4] & 0x000000ff) << 16) | ((rx_array[3] & 0x000000ff) << 8) | rx_array[2]
                #print(rx_array[i])    
            evefile.write("%s"%date)
            evefile.write(" ")
            evefile.write(str(eve_word))
            evefile.write("\n")
            
            #print ("**************** Mini-CoRTEx Event Data *************** ")
            #print(f"{eve_word:025b}")
            ...
            print ("Timestamp and Event",date)   
            #print (" ")   
            #value = int(eve_word, 16)
            print(f"{eve_word:032b}")
            #print (" ") 
    ...
    if(len(data) == 23):
        ...
        header = ((rx_array[1] & 0x00ff) << 8) | (rx_array[0] & 0x00ff)
        tailer = ((rx_array[22] & 0x00ff) << 8) | (rx_array[21] & 0x00ff)
        if(header == 0xaaaa and tailer == 0xd6d6):
            for i in range(2,21):
                mon_array.append(rx_array[i])    
                #print(rx_array[i])    
            monfile.write("%s"%date)
            monfile.write(" ")
            monfile.write(" ".join(map(str,mon_array)))
            monfile.write("\n")
            
            print ("**************** Mini-CoRTEx Data Monitor *************** ")
            print (" ")
            #time_stamp = int(round(time.time() * 1000))
            ...
            print (" ")   
            print ("******** Count rates ********")
            print ("ChannelX:0:",mon_array[0])
            print ("ChannelX:1:",mon_array[1])
            print ("ChannelX:2:",mon_array[2])
            print ("ChannelX:3:",mon_array[3])
            print ("ChannelX:4:",mon_array[4])
            print ("ChannelX:5:",mon_array[5])
            print ("ChannelX:6:",mon_array[6])
            print ("ChannelX:7:",mon_array[7])
            print ("ChannelX:8:",mon_array[8])
            print ("ChannelY:9:",mon_array[9])
            print ("ChannelY:10:",mon_array[10])
            print ("ChannelY:11:",mon_array[11])
            print ("ChannelY:12:",mon_array[12])
            print ("ChannelY:13:",mon_array[13])
            print ("ChannelY:14:",mon_array[14])
            print ("ChannelY:15:",mon_array[15])
            print ("ChannelY:16:",mon_array[16])
            print ("ChannelY:17:",mon_array[17])
            print ("Trigger rate:",mon_array[18])        
            print (" ")   
            print (" ") 
            
    time.sleep(0.5)

            #eve_array.append(((rx_array[3] & 0x00ff) << 8) | (rx_array[2] & 0x00ff))                                              
